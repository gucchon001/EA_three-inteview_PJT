...
指導管理ポータルでは、指導モニタリング管理表だけを正としない。パーサー実装時は、以下の責務を前提にする。

| データ | 正とするソース | パーサーでの扱い |
|---|---|---|
| 生徒・契約・コース等の基本情報 | FileMaker / 既存基幹情報 | Sheets値は移行初期値・差分検知に使う。将来はポータルDBを正にする |
| 実施時間・指導報告書・指導実績 | マイページ（指導報告書）BigQuery | Sheetsのチェック/集計値より優先する。列定義確認が必要な場合はBigQuery MCPで確認する |
| 面談/教師MTG/学習面談ステータス | 指導モニタリング管理表 | 移行期のステータス管理として取り込む |
| 学習計画表 | Google Sheets → 将来ポータルDB | 学習計画表作成MVP組み込み後、studentシートと合わせてSheets正本を廃止する |

契約時間充足率は、分母をFileMaker/基幹情報由来の月次契約時間、分子をBigQuery由来の実施時間として再計算する。Sheets上の値は移行期の検算・差分検知に使い、DB保存時の最終値はポータル側で算出する。

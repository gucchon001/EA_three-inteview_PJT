...
| 項目 | 値 |
|---|---|
| 最終更新 | 2026-05-10（Phase 0／DESIGN・モック `/mock` スキャフォールド追記） |
| ブランチ | `main` |
...
- ✅ **Phase 0（ドキュメント初版）**: PO 送付用パッケージ・PJC・WBS の Markdown 正本 — [docs/phase0/PO_REVIEW_PACKAGE.md](docs/phase0/PO_REVIEW_PACKAGE.md)／[docs/pjc/](docs/pjc/)／[docs/wbs/](docs/wbs/)（**PO 決裁・v0.4 反映まで完了ではない**）
- ✅ **モック UI スキャフォールド**: ルート [DESIGN.md](DESIGN.md)（仮デザイントークン）、[docs/web-app/mock-ui-boundary.md](docs/web-app/mock-ui-boundary.md)、`src/eb_app/`（`EB_ENABLE_MOCK_UI=1` で `GET /mock/...`）

### 未完了 / 次

優先度順:

1. **Phase 0: 設計確定**（2週想定）— **進行中**
   - **PO へ送付する統合パッケージ（正本）**: [docs/phase0/PO_REVIEW_PACKAGE.md](docs/phase0/PO_REVIEW_PACKAGE.md)（v0.3 §9 と parser-spec §10 を P-1〜P-10 に統合済み）
   - **PJC（Markdown 正本）**: [docs/pjc/eb_juku_instruction_monitoring_charter.md](docs/pjc/eb_juku_instruction_monitoring_charter.md) — スプレッドシート化する場合スキル `project-charter`
   - **WBS（Markdown 正本）**: [docs/wbs/eb_juku_instruction_monitoring_wbs.md](docs/wbs/eb_juku_instruction_monitoring_wbs.md) — シート転記時スキル `wbs`
   - PO 回答後: 要件 **v0.4** および [parser-spec.md](docs/sheets-migration/parser-spec.md) §10 の更新
...
- `docs/phase0/PO_REVIEW_PACKAGE.md` — Phase 0 PO 向け統合レビュー
- `docs/pjc/eb_juku_instruction_monitoring_charter.md` — プロジェクトチャーター（Phase 0）
- `docs/wbs/eb_juku_instruction_monitoring_wbs.md` — WBS（Phase 0）
- `docs/sheets-migration/README.md` — Phase -1 索引
- `docs/sheets-migration/parser-spec.md` — Phase 3 実装の入力
- `DESIGN.md` — Web UI デザイントークン仮置き
- `docs/web-app/mock-ui-boundary.md` — `/mock` ルート境界・起動例
...
- **Phase 0 で新規作成済み**: `docs/phase0/`、`docs/pjc/*.md`、`docs/wbs/*.md`、`docs/web-app/`、`DESIGN.md`、`src/eb_app/`、`requirements-app.txt`、`pytest.ini`（Web アプリ用）
...
一覧・決裁ログは **[docs/phase0/PO_REVIEW_PACKAGE.md](docs/phase0/PO_REVIEW_PACKAGE.md)** を正とする（元ソースは v0.3 §9／parser-spec §10）。
